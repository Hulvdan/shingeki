 ...
 *
 *
 *   A simple particle system built with and for raylib, to be used as header only
 *library.
 *
 *
 *   FEATURES:
 ...
 *       If not defined, the library is in header only mode and can be included in other
 *headers or source files without problems. But only ONE file should hold the
 *implementation.
 *
 *   LICENSE: zlib/libpng
 *
 *   libpartikel is licensed under an unmodified zlib/libpng license, which is an
...
/**  TODOs
 *
 * 0) MAYBE switch to purely function pointer based system.. handle Init, Update, Draw
 * etc. all as function pointers and make it possible to use a custom one. See current
 * handling of Particle deactivation functions as example.
 *
 */
...
Vector2 NormalizeV2(Vector2 v) {
    ...
    Vector2 res = {v.x / len, v.y / len};
    return res;
}

Vector2 RotateV2(Vector2 v, float degrees) {
    float   rad = degrees * DEG2RAD;
    Vector2 res = {cos(rad) * v.x - sin(rad) * v.y, sin(rad) * v.x + cos(rad) * v.y};
    return res;
}
...
struct EmitterConfig {
    ...
    FloatRange
        velocity;  // The possible range of the particle velocities.
    ...
    FloatRange
        originAcceleration;   // An acceleration towards or from (centrifugal) the origin.
    ...
    bool (*particle_Deactivator)(struct Particle*);  // Pointer to a function that
                                                     // determines when a particle is
                                                     // deactivated.
};
...
Particle* Particle_New(bool (*deactivatorFunc)(struct Particle*)) {
    ...
    p->externalAcceleration = Vector2Zero();
...
void Particle_Init(Particle* p, EmitterConfig* cfg) {
    ...
    p->velocity = Vector2{res.x * randv, res.y * randv};
...
void Particle_Update(Particle* p, float dt) {
    ...
    Vector2 toOrigin
        = NormalizeV2(Vector2{p->origin.x - p->position.x, p->origin.y - p->position.y});
...
bool Emitter_Reinit(Emitter* e, EmitterConfig cfg) {
    if (cfg.capacity > e->config.capacity) {
        ...
        Particle** newParticles
            = (Particle**)realloc(e->particles, cfg.capacity * sizeof(Particle*));
...
bool ParticleSystem_Register(ParticleSystem* ps, Emitter* emitter) {
    ...
    if (ps->length >= ps->capacity) {
        // Double capacity.
        Emitter** newEmitters
            = (Emitter**)realloc(ps->emitters, 2 * ps->capacity * sizeof(Emitter*));
        if (newEmitters == NULL) {
            return false;
        }
